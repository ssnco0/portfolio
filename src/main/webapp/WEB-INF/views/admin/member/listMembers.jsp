		...
		<div id="layoutSidenav_content">
			<main>
				<div class="container-fluid">
					<h1 class="mt-4">일반 사용자 관리</h1>
					<ol class="breadcrumb mb-4">
						<li class="breadcrumb-item"><a href="index.html">Dashboard</a></li>
						<li class="breadcrumb-item active">일반 사용자 관리</li>
					</ol>

					<div class="card mb-4">
						<div class="card-header">
							<form id="UserForm" method="post"
								action="${contextPath}/admin/member/CusUserMembers.do">
								<div class="input-group">
									<input type="search" name="cusid" class="form-control rounded"
								...
								<table class="table table-borderless" id="dataTable" width="100%" cellspacing="0">
									...
									<tbody>
										<c:forEach var="member" items="${membersList}">
											<tr align="center">
												<td>${member.cusid}</td>
												<td>${member.cusphone}</td>
												<td>${member.cuspassword}</td>
												<td>${member.cusemail}</td>
												<td>${member.cusname}</td>
												<td>${member.cusbirth}</td>
												<td>${member.cusadress}</td>
												<td>${member.cussignupdate}</td>
												<td>${member.cusadv}</td>
												<td>${member.cusqrno}</td>
												<td>${member.custotmil}</td>
												<td><a
													href="${contextPath}/admin/member/removeCusUserById.do?cusid=${member.cusid }">삭제하기</a></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
